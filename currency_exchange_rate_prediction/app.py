...
change_1=float(data_day2['Units per USD'])-float(data_day1['Units per USD'])
change_7=float(data_day7['Units per USD'])-float(data_day1['Units per USD'])
change_15=float(data_day15['Units per USD'])-float(data_day1['Units per USD'])
change_365=float(data_day365['Units per USD'])-float(data_day1['Units per USD'])

price_day1=float(data_day1['Units per USD'])
...
def index():
    ...
    actual_chart = go.Scatter(y=inr_df["Units per USD"], x=inr_df["Date"], name= 'Data')
...
def submit_data():
    ...
    df= inr_df.drop(['Currency', 'Name', 'USD per unit'], axis=1)
    ...
    final_df_1 = final_df_1.rename(columns={'yhat': 'Units Per USD', 'ds':value})
